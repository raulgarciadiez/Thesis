...
rect = matplotlib.patches.Rectangle((670.,-5.), 950, 7., color=colorgrey,alpha=0.15)
ax1.add_patch(rect)
...
ax2.errorbar(suffixes, crystal_const,yerr=err_crystal_const,fmt='o',color=color2)
ax2.hlines(crystal_const_av,min(suffixes),max(suffixes),linewidth=1.5, linestyle=':',color=color2,label='Average Crystal size = (%.2f) nm'%(crystal_const_av))

#ax1.errorbar(filedata[:,0],filedata[:,1]/1.e13,yerr=filedata[:,2]/1.e13,color=color1,fmt='o')
ax1.set_xlabel('Osmolality / mOsm kg$^{-1}$',size=14)
ax2.set_ylabel('Aggregate size / nm', color=color2,size=14,rotation=270,labelpad=20)
for tl in ax1.get_yticklabels():
    tl.set_color(color1)
ax1.vlines(670.,-5.,2.,linewidth=3., linestyle='--',color=colorgrey)
ax2.set_ylim([24.,70.])
...
ax1.errorbar(suffixes, 100*(average-p_list[:,0])/average,yerr=cov_list[:,0]/average,color=color1,fmt='p')
ax1.hlines(0,200,1550,color=color1,linewidth=1.5, linestyle=':')
ax1.set_ylabel(r'Diffraction peak deviation from $q=$%.2f nm$^{-1}$'%average+' / $\%$', color=color1,rotation=90, labelpad=5,size=14)
...
ax1.text(950, 1.1, "Osmotic Shrinkage", ha="center", va="center", rotation=0,
            size=17,
            bbox=bbox_props)
            
plt.savefig(dir_name+'/osmolality_effect_crystal_size.pdf', bbox_inches='tight', pad_inches=0.1,dpi=500)	
plt.show()
