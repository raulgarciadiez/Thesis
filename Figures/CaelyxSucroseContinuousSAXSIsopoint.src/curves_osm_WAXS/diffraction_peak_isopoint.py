...
rect = matplotlib.patches.Rectangle((670.,1.05), 950, 2.3, color=colorgrey,alpha=0.15)
ax1.add_patch(rect)
#ax3 = fig.add_subplot(111, xlim=(750,1550), ylim=(1.05e13,2.313), autoscale_on=False, bg_color='red')

ax1.errorbar(filedata[:,0],filedata[:,1]/1.e13,yerr=filedata[:,2]/1.e13,color='red',fmt='o',markersize=8)
ax1.set_xlabel('Osmolality / mOsm kg$^{-1}$',size=14)
ax1.set_ylabel('Intensity at $q=$0.122 nm$^{-1}$ / arb. u.', color=color1,size=14)
for tl in ax1.get_yticklabels():
    tl.set_color(color1)
ax1.vlines(670.,1.2,3.2,linewidth=3., linestyle='--',color=colorgrey)
ax1.set_ylim([1.2,2.9])
...
ax1.text(950, 2.75, "Osmotic Shrinkage", ha="center", va="center", rotation=0,
            size=17,
            bbox=bbox_props)
            
plt.savefig(dir_name+'/osmolality_effect_isopoint.pdf', bbox_inches='tight', pad_inches=0.1,dpi=500)	
plt.show()
