...
Considering that the refractive index is expressed generally by $n = 1 - \delta +i \beta$ and $\delta<10^{-3}$ in the X-ray regime\citep{henke_x-ray_1993}
, refraction effects can be neglected in scattering experiments because $\Re(n)$ is very close but smaller than unity.
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Scheme of an scattering event at a position $\vec{r}$. Geometrical definition of the momentum transfer vector}
...
\begin{figure}%[htbp]
	...
	\caption{Variation of the solvent electron denstiy represented by the electron density profile of a spherical core-shell particle: a) The contrast of both core and shell components is high, while in b) the solvent electron density is increased to match the shell's. The intensity scattering will be produced only be the core, due to the matching condition of the shell.}
\end{figure}
...
In a system measured at $N$ different solvent electron densities i.e. contrasts, the shape factor at each $q$-value can be calculated by solving the following matrix equation:
