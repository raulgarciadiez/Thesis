...
\begin{equation}
	f(q,R)=  \Delta\eta F_{\text{sph}}(q,R) +  \Delta\rho\left[ F_{\text{sph}}(q,R)-F_{\text{sph}}(q,R_{\text{core}}) \right] ,
\label{eq:ff_cs}
\end{equation}
...
Depending on the synthesis of the particles, the interface between the different phases might show a linear gradient of the electron densities of the components. Analogoulsy to expression \ref{eq:multicore-shell}, the form factor of a multicomponent spherical particle with a linear gradient interface is

\begin{equation}
f\left(q,R \right)=\sum_{\left[0,N-1\right]}\left[m_i \left( F_{\text{lin}}(q,R_{i+1})-F_{\text{lin}}(q,R_{i}) \right) +b_i \left( F_{\text{sph}}(q,R_{i+1})-F_{\text{sph}}(q,R_{i}) \right) \right]
\end{equation}
where $m_i=\sfrac{\rho_{i+1}-\rho_{i}}{R_{i+1}-R_{i}}$ and $b_i=\left( \rho_i-\rho_{\text{solv}} \right)-R_im_i$ and the linear form factor is defined by
\begin{equation}
F_{\text{lin}}(R,q)=4\pi\frac{ \left( 2qR\sin(qR)+2\cos(qR)-(qR)^2\cos(qR) \right)}{q^4}
\end{equation}
