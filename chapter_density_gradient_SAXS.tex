...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Solvent density along the gradient capillary vertical axis at different diffusion times, calculated from the transmission measurements at 5500 eV of 100 nm polystyrene particles. The corresponding X-ray transmission is shown on the right axis, revealing the low transmittances of the filled capillary at low energies.}
...
\begin{figure}%[htbp]
	\centering
		\subfloat[Calculated transmittance]{\resizebox{0.44\linewidth}{!}{\input{Figures/EnergiesTransmissionCalibration}}\label{fig:EnergiesTransmissionCalibration}}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Experimental scattering curves of PS-COOH nanoparticles for different suspending medium electron densities measured between 78 and 93 minutes after the inception of the density gradient. The dashed line shows the experimental background, containing scattering contributions from the capillary and the pure solvent.}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{The thick red line shows the scattering curve measured at $\rho_{\text{solv}}$ = 345.4 nm$^{-3}$, close to the match point, and the dotted line displays the experimental background. The symbols with errorbars show the background corrected scattering curve.}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{ The simulated scattering curves from the core-shell model fit at three selected contrasts $\rho_0-\rho_{\text{solv}}$ are shown as lines together with the experimental data points. In the inset, the electron density profile corresponding to the fitted core-shell form factor is displayed.}
...
Although the first isoscattering point is clearly visible in figure \ref{fig:KiskerContinuousSAXS}, a model-free approach like the isoscattering point requires of a more precise positioning and a quantitative evaluation. For this purpose, the relative standard deviation $\sigma_r$ of the 40 measured curves at each \(q\) is calculated according to
\begin{equation}
...
\begin{figure}%[h]
    ...
    \subfloat[Relative standard deviation]{
        \resizebox{0.44\linewidth}{!}{\input{Figures/KiskerIsopoint}}
        \label{fig:KiskerIsopoint}
    }
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Deviation from the $I(0)$ values used in the data evaluation: The Guinier approximation overstimates the experimental values, while the intensity at $q=0.03$ nm$^{-1}$ understimates the zero-angle intensity.}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Experimental squared radius of gyration as a function of the solvent electron density. Equation \eqref{eq:gyration} is fitted to the data and shown as a thick line. The vertical and horizontal asymptotes correspond to $\rho_0$ and $\tilde R^2_{g,c}$ respectively.}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Experimental zero-angle intensity as a function of the solvent electron density. The function corresponding to equation \eqref{eq:I0} is fitted to the data and shown as a thick line. The minimum in the parabola corresponds to $\rho_{\text{solv}}=\rho_0$.}
...
\begin{table}
\caption{Comparison of the different methods presented in section \ref{sec:KiskerResultsEvaluation} to evaluate SAXS contrast variation data.}
\begin{tabular}{l|ccc}
 & \( R\) (nm)    & \(\rho_0\) (nm\(^{-3}\)) & \(p_d\) (\(\%\))\\
\hline
...
\subsubsection{Applicability of the technique and comparison with other contrast variation approaches}
...
\begin{figure}%[htbp]
	\centering
		...
		\caption{Concentration gradient of 12 nm silica particles mesaured at 8000 eV. The large size of the colloids provides a longer diffusion time than a typical contrast agent like sucrose.}
