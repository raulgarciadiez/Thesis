...
class Fit_analysis:
	...
	def plot_fig(self):
		...
		for i,(suffix,color) in enumerate(zip(suffixes_plot,colors)):
			...
			q=np.arange(0.03,1,0.0001)
			result=np.empty(shape=(len(q),2))
			result[:,0]=q
			result[:,1]=N*(s.compute(q,Rho_solvent+tuple(self.p))+C0+C4*q**(-alpha))
			
			np.savetxt('Coreshell_fit_%.2f.dat'%(suffix),result)
