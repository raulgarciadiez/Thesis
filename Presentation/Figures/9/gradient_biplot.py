...
#        x = np.arange(n)
    ...
    rc('font',**{'family':'serif','serif':['Palation'], 'size':28, 'weight':'bold'})
    rc('text', usetex=True)
    ...
    samp = Slider(axslider, 'Time (min)', 0., maxtime, valinit=0,color='grey',alpha=0.3,valfmt='%i')
...
#    xcolor, ycolor = verts[:, 0], verts[:, 1]
    ...
    # Sthetical tuning of plot    
    ax2.set_ylim([min(x),max(x)])
    ax2.set_xlim([990,1093])
    ax2.set_xlabel(r"\textbf{Density / g L}$\boldsymbol{^{-1}}$")
    ax2.set_ylabel(r"\textbf{Height / mm}",rotation=270, labelpad= 40)#fontsize=20,
...
if __name__ == '__main__':
    ...
    for (i,time) in enumerate(time_range):
        ...
        fig = plot_biplot(x_i,rho_i, rhomin, rhomax)
        name = basename+str(i).zfill(4)
        fig.savefig(name,bbox_inches='tight',facecolor='None')#,dpi=150)
