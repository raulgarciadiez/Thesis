...
for isotropically scattering samples,  a data reduction step is performed, known as radial "integration", "averaging" or "binning".

2D to 1D: a reduction in dimensionality of the data usually accompanies this procedure (e.g. from 2D images to 1D plots), 

q-binning: by grouping and averaging pixels with similar scattering angle $q$ irrespective of their azimuthal angle on the detector (denoted $\phi$).

advantadge of q-binning: the relative data uncertainties become smaller for the averaged data.

the spacing between the bin edges. Normally, this is chosen either uniform or logarithmically spaced (with more data points at low values).

for radial averaging the many datapoints Ij collected from each pixel on the the detector are reduced into a small number of q-bins Iqbin 

each measured datapoint collected between the bin edges (class limits) qn and qn+1 is averaged 

CALCULATION OF THE EROR IN Q

the standard deviation between similar pixels in a group (c.q. bin) can provide a good estimate for the actual uncertainty on the average value if this standard deviation exceeds the photon counting statistics-based estimate propagated until this step (The photon counting (Poisson) statistics 

sample standard deviation in the bin IS BETTER THAN POISON STATISTICS, IS AN TYPE OF RESIDUALS

USING A MASK TO FIND HOT PIXEL

finding dead pixels (disturbing pixels)

low-pass filter (Nyquist–Shannon sampling theorem) and then it is used a gaussian kernel
...
this procedure can be used to find the center, as it minimized the standard deviation error for the best possible mask

typical and easy center fitting: using a standard calibrant such as AgBehe
...
-correction for spherical angles ---- LOOK GUDRUN EXPLANATIOn
